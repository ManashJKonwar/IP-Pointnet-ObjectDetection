...
import pickle
import numpy as np
from utility.KITTI.kitti_object import kitti_object, get_lidar_in_image_fov
from utility.KITTI.kitti_util import compute_box_3d
...
def extract_frustum_data(idx_filename, 
                        ...
                        augmentX=1, 
                        type_whitelist=['Car'], 
                        dataset_folder=r'datasets\KITTI'):
    ...
    pos_cnt = 0
    all_cnt = 0
    for data_idx in tqdm.tqdm(data_idx_list, desc='Extracting point clouds and annotations in frustum for filename: %s'%(str(idx_filename))):
        # print('------------- ', data_idx)
...
def extract_frustum_data_rgb_detection(det_filename, 
    ...
    for det_idx in tqdm.tqdm(range(len(det_id_list)), desc='Extracting point clouds and annotations in frustum for filename: %s'%(str(det_filename))):
